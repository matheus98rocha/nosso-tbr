...
- **RN35 - Descoberta e layout:** A rota `/register` não faz parte da navegação principal nem do layout `(main)`; o acesso é apenas por URL direta com **convite** na query string: `/register?invite=<token>`, onde `<token>` deve coincidir com o segredo configurado no servidor (`REGISTER_INVITE_SECRET`). Sem `invite` válido na URL, a UI explica que o cadastro é só por link de convite. Trocar o segredo no ambiente gera um novo link válido e invalida os anteriores.
- **RN36 - Fluxo de persistência:** O endpoint `POST /api/auth/register` exige no corpo o campo `invite` (mesmo valor do token da URL). Se `REGISTER_INVITE_SECRET` não estiver definido no servidor, a rota responde 503. Se `invite` não coincidir com o segredo (comparação em tempo constante), responde 403. Caso válido, cria o usuário no Supabase Auth (`signUp`) e em seguida faz upsert em `public.users` com `id` igual ao `auth.users.id`, preenchendo `display_name` e `email`.
- **RN37 - Validação (API + formulário):** Os campos enviados à API (`email`, `password`, `display_name`, `invite`) são obrigatórios: `email` com formato válido; `password` com mínimo de 8 caracteres; `display_name` com trim, mínimo de 2 e máximo de 200 caracteres; `invite` não vazio. A rota usa `registerUserBodySchema`; o formulário usa `registerUserFormSchema`, que inclui `password_confirm` obrigatório e deve coincidir com `password` (o campo de confirmação não é enviado ao servidor além de `invite` e dos campos de perfil). As regras dos campos persistidos em `users` ficam centralizadas em `registerFieldsSchema` no mesmo módulo de validação.
